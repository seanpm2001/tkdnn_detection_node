...
def test_learner_confidence():
    learner = l.Learner()
    assert len(learner.low_conf_box_observations) == 0
    ...
    assert len(learner.low_conf_box_observations) == 1, 'Detection should be stored'

    active_learning_cause = learner.add_box_detections([dirt_detection])
    assert len(learner.low_conf_box_observations) == 1, f'Detection should already be stored'
    assert active_learning_cause == []

    active_learning_cause = learner.add_box_detections([conf_too_low_detection])
    assert len(learner.low_conf_box_observations) == 1, f'Confidence of detection too low'
    assert active_learning_cause == []

    active_learning_cause = learner.add_box_detections([conf_too_high_detection])
    assert len(learner.low_conf_box_observations) == 1, f'Confidence of detection too high'
    assert active_learning_cause == []
...
def test_add_second_detection_to_learner():
    ...
    assert len(learner.low_conf_box_observations) == 2, 'Second detection should be stored'
...
def test_forget_old_detections():
    ...
    assert len(learner.low_conf_box_observations) == 1

    learner.low_conf_box_observations[0].last_seen = datetime.now() - timedelta(minutes=30)
    learner.forget_old_detections()
    assert len(learner.low_conf_box_observations) == 1

    learner.low_conf_box_observations[0].last_seen = datetime.now() - timedelta(hours=1, minutes=1)
    learner.forget_old_detections()
    assert len(learner.low_conf_box_observations) == 0
